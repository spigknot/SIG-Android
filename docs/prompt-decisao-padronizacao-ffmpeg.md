...
- Convenções dos repositórios: no Android, todo fonte de produção precisa de linha no `MODULE-MAP.md` e KDoc; o commit passa por um gate/pre-commit (nunca use `--no-verify`). No Windows, os testes são **unittest** descobertos por `scripts/release.py tests` (não pytest — o rótulo "pytest" é impreciso); o gate canônico é `python scripts/release.py {syntax,tests,ui-smoke,preflight}` seguido de `scripts/build_dev.py`, conforme o runbook `UPDATE.md`. O painel é o arquivo grande `src/ffmpeg_tools_panel.py` (evite refatorações amplas — extraia só o necessário).
