...
$files = if ([string]::IsNullOrWhiteSpace($raw)) { @() } else { @($raw -split "\r?\n" | Where-Object { -not [string]::IsNullOrWhiteSpace($_) }) }
...
$untrackedFiles = if ([string]::IsNullOrWhiteSpace($untrackedRaw)) { @() } else { @($untrackedRaw -split "\r?\n" | Where-Object { -not [string]::IsNullOrWhiteSpace($_) }) }
...
if ($Json) {
    ...
    if ($status -eq "pass") {
        ...
        if ($trackedCount -gt 0) { [Console]::Error.WriteLine(("commit bloqueado: {0} arquivo(s) tracked fora do índice" -f $trackedCount)) }
        if ($untrackedBuildInputCount -gt 0) { [Console]::Error.WriteLine(("commit bloqueado: {0} entrada(s) do Gradle não rastreada(s)" -f $untrackedBuildInputCount)) }
    }
}
